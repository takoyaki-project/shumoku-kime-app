...
// このスクリプト相当の変換を手動で作り直すこと。
const EVENTS_MASTER = {
  "_note": "甲一（甲子園一番町）向け種目マスタ。種目名は『去年のGoogleフォーム（今年もそのまま使用）』の実際の文言を正とする。募集人数は2026年度PDF按分（甲一分）を正とする（フォームの古い人数は使わない）。",
  "normalize_rule": "先頭の☆記号・丸数字・半角/全角数字・「．」「.」「、」を除去し、さらに文字列内部の空白（半角・全角スペース）もすべて除去してから比較する（例：『☆２２．地区対抗リレー（中学生以上　女子）』→『地区対抗リレー（中学生以上女子）』、『１８．徒競走（　就学前児童）』→『徒競走（就学前児童）』）",
  ...
  "events": [
    ...
    },
    {
      ...
      "_note": "フォームの年齢選択肢は『小学１～３年生』という学年帯までしか分からず、1・2・3年の個別内訳は判定できない。1〜3年生全体で1つの定員として扱う"
    },
    {
      "id": "e05",
      "name": "ペアで協力",
    ...
    },
    {
      ...
      "_note": "フォームの年齢選択肢は学年帯（小学１～３年生／小学４～６年生）までしか分からず、個別学年の内訳は判定できない。小学生女子全体で1つの定員として扱う"
    },
    {
      "id": "e14",
      "name": "徒競走",
    ...
    },
    {
      ...
      "scored": true,
      "description": "全員半周のリレー",
      "_note": "フォームの年齢選択肢は学年帯（小学１～３年生／小学４～６年生）までしか分からず、個別学年の内訳は判定できない。小学生男子全体で1つの定員として扱う"
    },
    {
      "id": "e16",
      "name": "徒競走（就学前児童）",
    ...
    },
    {
      ...
      "_note": "内訳の合計は2+6+1=9でcapacity_totalと一致させてある。『小学生(1〜6年)』は個別学年に分けず、まとめて1つの定員として扱う（フォームの年齢選択肢が学年帯までしか分からないため）"
    },
    {
      "id": "e18",
      "name": "フットボールレース",
  ...
  "_relay_note": "地区対抗リレー（e13,e15,e20,e21）は、以前は独立3択で管理する設計だったが、実際のフォームでは他の種目と同じ『出場希望』1問に統一された。上記eventsの各エントリがそのまま地区対抗リレーであり、別のrelayオブジェクトは不要。",
  "age_categories": [
    "就学前児童（保護者同伴で競技に参加）",
    "就学前児童（一人で競技に参加できる）",
    "小学１～３年生",
  ...
  "priority_rule": "回答者ごとに『出場希望』をチェックした種目の総数を数え、数が少ない人ほど優先度を高くする。出られる種目数の申告や重み付けの質問は存在しない。"
};
